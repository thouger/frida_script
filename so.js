    ...
    Java.perform(function () {
        ...
        for (var i = 0; i < process_Obj_Module_Arr.length; i++) {
            ...
            if (name.indexOf('.so') != -1) {
                const hooks = Module.load(name);
                var Imports = hooks.enumerateImports();
                for (var i = 0; i < Imports.length; i++) {
                    var Import = Imports[i];
                    //so名字，函数类型，函数名称，属于的模块，函数地址
                    console.log("so:", name, "type:", Import.type, 'name:', Import.name, "module:", Import.module, "address:", Import.address)
                }
            }
        }
    });
