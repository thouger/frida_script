...
function inspectObject(obj, input) {
    ...
    input = input.concat("\r\nInspecting Fields: => ", isInstance, " => ", obj_class.toString());
    input = input.concat("\r\n")
...
function main() {
    Java.perform(function () {
        ...
        */
        ...
        hook("com.appsflyer.internal.AFa1nSDK$30218$AFa1wSDK$25697","","com.appsflyer.internal.AFa1nSDK$30218$AFa1wSDK$25697");     


        //我的测试
        // traceClass("com.gst.unidgbapp.Person")
        // hook("com.appsflyer.internal.AFa1nSDK$30218$AFa1wSDK$25697", "")
    })
}


function getMethod(clz, methodName){
    var methods = clz.getDeclaredMethods()
    for(var idx = 0; idx < methods.length; ++idx){
        var name = methods[idx].getName()
        if(name == methodName){
...
function hookAppCreate() {
    Java.perform(function () {
        ...
        Application.attach.implementation = function (context) {
            ...
            console.log("app has:" + loadedClasses.length + " classes")
            console.log("start up r0tracer")
            main()
        }
    })
}
