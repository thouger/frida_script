...
*/
	...
	try {
		...
		send("registerClass from X509TrustManager >>>>>>>> " + e.message);
	}
	...
	try {
		...
		CertificatePinner.check.overload('java.lang.String', 'java.util.List').implementation = function() {
			...
			send('OkHTTP 3.x check() called. Not throwing an exception.');
		}
	...
	try {
		...
		PinningTrustManager.checkServerTrusted.implementation = function() {
			...
			send('Appcelerator checkServerTrusted() called. Not throwing an exception.');
		}
	...
	try {
		...
		OkHttpClient.setCertificatePinner.implementation = function(certificatePinner) {
			// do nothing
			send("OkHttpClient.setCertificatePinner Called!");
			return this;
		};
		...
		CertificatePinner.check.overload('java.lang.String', '[Ljava.security.cert.Certificate;').implementation = function(p0, p1) {
			// do nothing
			send("okhttp Called! [Certificate]");
			return;
		};
		CertificatePinner.check.overload('java.lang.String', 'java.util.List').implementation = function(p0, p1) {
			// do nothing
			send("okhttp Called! [List]");
			return;
		};
	} catch (e) {
		send("com.squareup.okhttp not found");
	}
	...
	//     send("TrustManagerFactory getTrustManagers invoked");
	...
	HttpsURLConnection.setDefaultHostnameVerifier.implementation = function(hostnameVerifier) {
		send("HttpsURLConnection.setDefaultHostnameVerifier invoked");
		return null;
	};
	/* libcore/luni/src/main/java/javax/net/ssl/HttpsURLConnection.java */
	/* public void setSSLSocketFactory(SSLSocketFactory) */
	HttpsURLConnection.setSSLSocketFactory.implementation = function(SSLSocketFactory) {
		send("HttpsURLConnection.setSSLSocketFactory invoked");
		return null;
	};
	/* libcore/luni/src/main/java/javax/net/ssl/HttpsURLConnection.java */
	/* public void setHostnameVerifier(HostnameVerifier) */
	HttpsURLConnection.setHostnameVerifier.implementation = function(hostnameVerifier) {
		send("HttpsURLConnection.setHostnameVerifier invoked");
		return null;
	};
	...
	try {
		...
		send("registerClass from hostnameVerifier >>>>>>>> " + e.message);
	}
	...
	try {
		...
		AbstractVerifier.verify.overload('java.lang.String', '[Ljava.lang.String', '[Ljava.lang.String', 'boolean').implementation = function() {
			send("httpclientandroidlib Hooks");
			return null;
		}
	} catch (e) {
		send("httpclientandroidlib Hooks not found");
	}
...
***/
	...
	//         send("TrustManagerImpl checkTrusted called");
	//         //Generics currently result in java.lang.Object
	//         return Arrays.asList(chain);
	//     }
	//
	// } catch (e) {
	//     send("TrustManagerImpl checkTrusted nout found");
	// }

	try {
		// Android 7+ TrustManagerImpl
		TrustManagerImpl.verifyChain.implementation = function(untrustedChain, trustAnchorChain, host, clientAuth, ocspData, tlsSctData) {
			send("TrustManagerImpl verifyChain called");
		...
		send("TrustManagerImpl verifyChain nout found below 7.0");
	}
	// OpenSSLSocketImpl
	try {
		var OpenSSLSocketImpl = Java.use('com.android.org.conscrypt.OpenSSLSocketImpl');
		OpenSSLSocketImpl.verifyCertificateChain.implementation = function(certRefs, authMethod) {
			send('OpenSSLSocketImpl.verifyCertificateChain');
		}

		send('OpenSSLSocketImpl pinning')
	} catch (err) {
	...
	try {
		...
		Activity.verify.overload('java.lang.String', 'javax.net.ssl.SSLSession').implementation = function(str) {
			send('Trustkit.verify1: ' + str);
			return true;
		};
		Activity.verify.overload('java.lang.String', 'java.security.cert.X509Certificate').implementation = function(str) {
			send('Trustkit.verify2: ' + str);
			return true;
		};

		send('Trustkit pinning')
	} catch (err) {
